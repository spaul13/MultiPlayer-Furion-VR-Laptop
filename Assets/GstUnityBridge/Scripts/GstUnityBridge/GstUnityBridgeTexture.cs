   ...
   public void InitializeSetupPlay()
     ...
     * private void LateUpdate()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            Debug.Log("\n the current time before started playing = " + Time.realtimeSinceStartup);

            m_URI = "file:///D:/decoding_videos/of_Ionly/" + fID.ToString() + ".mp4";
            Setup(m_URI, m_VideoIndex, m_AudioIndex);
            Play();
            Debug.Log(" \n 1/fps = " + Time.unscaledDeltaTime*1000);
            Debug.Log("\n the time difference between playing = " + (Time.realtimeSinceStartup - start_time));
    ...
    */
    ...
    void Update()
    {
        ...
        if (false)
        {
            ...
            m_URI = "file:///C:/Users/spauldsnl/Documents/decoding_videos/viking_texas_Ionly/" + fID.ToString() + ".mp4"; //D:/decoding_videos/of_Ionly/"
            Setup(m_URI, m_VideoIndex, m_AudioIndex);
            Play();
            Debug.Log(" \n 1/fps = " + Time.unscaledDeltaTime * 1000);
            Debug.Log("\n the time difference between playing = " + (Time.realtimeSinceStartup - start_time));
    ...
    void LateUpdate()
    {
        ...
        if (m_Pipeline.GrabFrame(ref sz))
        {
            ...
            else
            {
                 ...
                 * if (change == 1)
                {
                    sum_start += Time.realtimeSinceStartup - start_time;
                    Debug.Log("\n the time difference between changing and grabbing a frame = " + (Time.realtimeSinceStartup - start_time));
                    count++;
                    Debug.Log("\n Avg = " + sum_start / count + ", total_time = " + sum_start + ", count = " + count);
                    change = 0;
                }
                */
            }
            ...
            if (m_FirstFrame)
            {
                Debug.Log("\n the adaptive bit rate is = " + m_AdaptiveBitrateLimit);
